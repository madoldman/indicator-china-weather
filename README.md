...
+ **刷新间隔**：自动刷新周期，单位分钟，默认 20（保存后立即按新间隔生效）

IP 定位失败或定位到的城市不在本地城市表中时，面板会显示明确的中文错误提示（不会静默回落到任何写死城市）。网络原因导致的定位失败会按 30 秒起逐次翻倍（封顶 5 分钟）的节奏自动重试，网络恢复后无需任何操作即可自行恢复；定位到的城市不在本地城市表中时属于配置问题，不会自动重试，此时可在设置中手动选择城市。
